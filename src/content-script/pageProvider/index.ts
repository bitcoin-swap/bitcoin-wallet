...
const log = (event, ...args) => {
  if (process.env.NODE_ENV !== 'production') {
    console.log(
      `%c [BitcoinWallet] (${new Date().toTimeString().slice(0, 8)}) ${event}`,
...
const channelName = script?.getAttribute('channel') || 'BITCOINWALLET' || 'UNISAT';
...
export class BitcoinWalletProvider extends EventEmitter {
...
declare global {
  interface Window {
    bitcoinWallet: BitcoinWalletProvider;
  }
}
...
if (!window.bitcoinWallet) {
  window.bitcoinWallet = new Proxy(provider, {
    deleteProperty: () => true
  });
}
